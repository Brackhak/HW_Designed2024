...
static const char* TX_Sit_Range = "Sit_Range";
static const char* sit_uart_state = "";
static bool sit_state = false;
...
void clock_position_motion_dectect(void)
{
  ...
  while (clock_status == WAIT)
  {
    ...
    if (val >= 5.0){
      ...
      clock_status = OK;
      timer_pause(TIMER_GROUP_1, TIMER_0);
      Serial.printf("GET NEW CLOCK POSITION! -> %d \n", sit_range);
      timer_set_counter_value(TIMER_GROUP_1, TIMER_0, 0);
    }
    Serial.println(val);
  }
...
static void tx_task(void *arg)
{
    ...
    while (1) {
        sit_uart_state = sit_state ? "a" : "A";
        sendData(TX_Sit_Range, sit_uart_state);
        Serial.println(sit_state);
